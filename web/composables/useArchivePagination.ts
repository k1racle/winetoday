...
export async function useArchivePagination<T extends { id: string | number }>(
...
): Promise<UseArchivePaginationResult<T>> {
  ...
  const filteredItems = computed<T[]>(() => {
    if (!options.excludeIds) return allItems.value;
    const exclude = typeof options.excludeIds === 'function' ? options.excludeIds() : options.excludeIds.value;
    return allItems.value.filter((i) => !exclude.has(i.id));
  });

  const { data: initialData, error: initialError } = await useAsyncData(key, () =>
    fetcher({ limit: itemsPerPage, offset: 0 }).catch(() => ({ items: [] as T[], total: undefined })),
  );
