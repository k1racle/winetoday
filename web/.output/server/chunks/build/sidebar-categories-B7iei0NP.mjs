import { computed, toValue, getCurrentInstance, onServerPrefetch, ref, shallowRef, nextTick, unref, toRef, useSSRContext, defineComponent, useAttrs, mergeProps } from 'vue';
import { g as useNuxtApp, i as asyncDataDefaults, a as useRuntimeConfig, d as createError, u as useHead } from './server.mjs';
import { ssrRenderAttrs, ssrRenderSlot } from 'vue/server-renderer';
import { z as defu, r as hasProtocol, C as withLeadingSlash, n as joinURL, p as parseURL } from '../_/nitro.mjs';
...
const noneRuntime$zWoCCQOQrWIY42w3o_OxrhQox0FFPXDgjVZDgeleGqE = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  getImage
...
const imageOptions = {
  ...
  providers: {
    ["none"]: { provider: noneRuntime$zWoCCQOQrWIY42w3o_OxrhQox0FFPXDgjVZDgeleGqE, defaults: {} }
  }
};
...
const __nuxt_component_1 = Object.assign(_sfc_main, { __name: "NuxtImg" });
...
SIDEBAR_CATEGORY_GROUPS.map((group) => group.label);
...
function sortBySidebarCategoryOrder(groups) {
  ...
  return groups.map((group) => ({ group, index: resolveGroupIndex(group.category) })).filter((entry) => entry.index !== null).filter((entry) => {
    if (usedGroups.has(entry.index)) {
      return false;
    }
    usedGroups.add(entry.index);
    return true;
  }).sort((left, right) => left.index - right.index).map((entry) => entry.group);
}
...
export { SIDEBAR_CATEGORY_GROUPS as S, __nuxt_component_1 as _, useMediaUrl as a, matchSidebarCategory as m, resolveSidebarCategoryLabel as r, sortBySidebarCategoryOrder as s, useAsyncData as u };
//# sourceMappingURL=sidebar-categories-B7iei0NP.mjs.map
