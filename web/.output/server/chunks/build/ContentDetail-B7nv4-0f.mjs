...
import { u as useMediaUrl, _ as __nuxt_component_2$1 } from './SidebarByCategory-CS-C-5mH.mjs';
import { defineComponent, withAsyncContext, computed, ref, mergeProps, withCtx, createTextVNode, toDisplayString, unref, useSSRContext } from 'vue';
...
import { u as useContentMeta, _ as __nuxt_component_1$2 } from './ArticleCard-BzTIibt-.mjs';
...
function renderNode(node) {
  ...
  const style = node.attrs?.textAlign && node.attrs.textAlign !== "left" ? ` style="text-align:${escapeHtml(node.attrs.textAlign)}"` : "";
  switch (node.type) {
    case "paragraph":
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(props.blocks, (block) => {
        ...
        if (block.type === "rich-text" || block.type === "html-editor" || block.type === "text") {
          _push(`<div class="prose prose-lg max-w-none text-foreground">${renderContent(block.content) ?? ""}</div>`);
          ...
          if (block.data?.caption || block.data?.source) {
            _push(`<figcaption class="mt-2 text-sm text-foreground/60">${ssrInterpolate(block.data.caption)} `);
            if (block.data.caption && block.data.source) {
              _push(`<span> / </span>`);
            } else {
              _push(`<!---->`);
            ...
            if (block.data.source) {
              _push(`<span>${ssrInterpolate(block.data.source)}</span>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</figcaption>`);
          ...
          ssrRenderList(block.data?.items, (item, i) => {
            ...
            if (item.source) {
              _push(`<figcaption class="mt-1 text-xs text-foreground/60">${ssrInterpolate(item.source)}</figcaption>`);
            } else {
              _push(`<!---->`);
            }
            _push(`</figure>`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(coverSrc) && __props.item.type !== "video") {
...
//# sourceMappingURL=ContentDetail-B7nv4-0f.mjs.map
