...
import { defineComponent, ref, computed, mergeProps, unref, withCtx, createVNode, toDisplayString, openBlock, createBlock, createCommentVNode, createTextVNode, useSSRContext, withAsyncContext } from "vue";
...
import { _ as __nuxt_component_1$1, u as useAsyncData } from "./NuxtImg-BJV0ypiM.js";
import { u as useMediaUrl } from "./useMediaUrl-BBfhl7w5.js";
import { u as useContentMeta, _ as __nuxt_component_1$2 } from "./ArticleCard-BaWiHPYX.js";
import { _ as __nuxt_component_0$2 } from "./VideoThumb-DMCw6jdJ.js";
      ...
      ssrRenderList(unref(displayedItems), (item) => {
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="shrink-0 pt-0.5 text-sm font-bold text-accent"${_scopeId}>${ssrInterpolate(formatDayMonth(item.publishedAt || item.createdAt))}</span><span class="min-w-0 text-sm font-medium leading-snug text-foreground group-hover:text-foreground"${_scopeId}>${ssrInterpolate(item.title)}</span>`);
              if (item.pinned) {
                _push2(`<span class="ml-auto shrink-0 self-center rounded bg-red-700 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white"${_scopeId}> Эксклюзив </span>`);
              } else {
                _push2(`<!---->`);
              }
            } else {
              return [
                ...
                createVNode("span", { class: "min-w-0 text-sm font-medium leading-snug text-foreground group-hover:text-foreground" }, toDisplayString(item.title), 1),
                item.pinned ? (openBlock(), createBlock("span", {
                  key: 0,
                  class: "ml-auto shrink-0 self-center rounded bg-red-700 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white"
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_NuxtLink, mergeProps({
        ...
        class: ["group relative flex min-w-0 flex-col justify-end overflow-hidden bg-foreground/10", {
          "aspect-video md:aspect-square": __props.size === "small" || __props.size === "medium" || __props.size === "large",
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (unref(typeLabel)) {
              _push2(`<span${_scopeId}>${ssrInterpolate(unref(typeLabel))}</span>`);
            } else {
              _push2(`<!---->`);
            }
            if (unref(category)) {
            ...
            return [
              ...
              createVNode("div", { class: "relative z-10 p-3 text-white md:p-6" }, [
                createVNode("div", { class: "mb-1.5 flex flex-wrap items-center gap-2 text-[10px] font-medium uppercase tracking-wider text-white/80 md:text-xs" }, [
                  unref(typeLabel) ? (openBlock(), createBlock("span", { key: 0 }, toDisplayString(unref(typeLabel)), 1)) : createCommentVNode("", true),
                  unref(category) ? (openBlock(), createBlock("span", { key: 1 }, toDisplayString(unref(category)), 1)) : createCommentVNode("", true),
                  unref(shortDate) ? (openBlock(), createBlock("span", { key: 2 }, toDisplayString(unref(shortDate)), 1)) : createCommentVNode("", true)
                ]),
                createVNode("h3", {
...
const __nuxt_component_1 = Object.assign(_sfc_main$3, { __name: "HeroCard" });
...
const __nuxt_component_2 = Object.assign(_sfc_main$2, { __name: "NewsThumbCard" });
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (__props.showTitle) {
              _push2(`<div class="relative z-10 p-5 text-white md:p-8"${_scopeId}><div class="mb-2 flex flex-wrap items-center gap-2 text-xs font-medium uppercase tracking-wider text-white/80"${_scopeId}><span${_scopeId}>Видео</span>`);
              if (unref(category)) {
                _push2(`<span${_scopeId}>${ssrInterpolate(unref(category))}</span>`);
              } else {
                _push2(`<!---->`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const { getHomepage, getArticles, getLatestByCategory } = useApi();
    ...
    const { data: fresh } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(
      "fresh",
      () => getArticles({ limit: 12 }).catch(() => ({ items: [] }))
    )), __temp = await __temp, __restore(), __temp);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(topItems).length) {
        _push(`<section class="mx-auto max-w-7xl px-4 py-4"><div class="grid grid-cols-1 gap-4 lg:grid-cols-4"><aside class="order-first w-full lg:order-none lg:col-start-4 lg:row-start-1 lg:row-span-3">`);
        _push(ssrRenderComponent(_component_FreshList, {
          items: unref(freshItems),
          class: "h-full"
        }, null, _parent));
        _push(`</aside><div class="hidden w-full lg:col-start-1 lg:col-span-2 lg:row-start-1 lg:row-span-2 lg:block">`);
        if (unref(topItems)[0]) {
        ...
        _push(`</div><!--[-->`);
        ssrRenderList(unref(topItems).slice(0, 3), (item, idx) => {
          _push(`<div class="${ssrRenderClass([idx === 0 ? "lg:hidden" : "lg:col-start-3 " + (idx === 1 ? "lg:row-start-1" : "lg:row-start-2"), "w-full"])}">`);
        ...
        if (unref(homepage)?.videos?.length) {
          _push(`<div class="w-full lg:col-span-3 lg:col-start-1 lg:row-start-3"><div class="mb-4 flex items-center justify-between"><div class="flex items-center gap-2"><svg class="h-5 w-5 fill-current text-accent" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"></path></svg><h2 class="font-heading text-xl font-bold uppercase tracking-wider">Видео</h2></div>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: "/videos",
      ...
      if (unref(mixedItems).length) {
        _push(`<section class="mx-auto max-w-7xl px-4 py-10"><h2 class="mb-6 font-heading text-2xl font-bold">Последние новости</h2><div class="grid items-start gap-4 lg:grid-cols-4"><div class="grid h-fit auto-rows-min content-start items-start gap-4 sm:grid-cols-2 lg:col-span-3 lg:grid-cols-3"><!--[-->`);
...
//# sourceMappingURL=index-D1-ASE3y.js.map
