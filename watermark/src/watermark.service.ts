...
export interface ApplyWatermarkRequest {
  ...
  /** Width of the rendered content block in CSS pixels. Used as the basis for
   *  the watermark size instead of the original photo width. */
  targetWidth?: number;
}

const DEFAULT_SETTINGS: Required<WatermarkSettings> = {
  opacity: 0.85,
  sizePercent: 18,
...
function resolveUploadsPath(filePath: string, uploadsDir: string): string {
...
function makeOutputPath(sourcePath: string): string {
