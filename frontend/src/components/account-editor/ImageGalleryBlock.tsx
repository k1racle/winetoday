...
import type { EditorGalleryBlock, UploadedAsset } from "./types";
...
type ImageGalleryBlockProps = {
  block: EditorGalleryBlock;
  assets: UploadedAsset[];
  onChange: (block: EditorGalleryBlock) => void;
  onUpload: (event: React.ChangeEvent<HTMLInputElement>) => Promise<void>;
};

export function ImageGalleryBlock({ block, assets, onChange, onUpload }: ImageGalleryBlockProps) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="grid gap-3">
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-zinc-600 dark:text-zinc-300">Заголовок галереи</label>
        <input
          type="text"
          value={block.title ?? ""}
      ...
      <div>
        ...
        {block.images.length > 0 ? (
          <div className="mt-2 flex flex-wrap gap-2">
            {block.images.map((id) => (
              <span key={id} className="rounded bg-emerald-100 px-2 py-1 text-xs text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200">
                ID: {id}
              </span>
            ))}
          </div>
