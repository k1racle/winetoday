...
type CoverPanelProps = {
  form: Pick<FormState, "cover" | "archiveCover" | "coverSource">;
  assets: UploadedAsset[];
  activePanel: ActiveMediaPanel;
  onOpenPanel: (kind: "cover" | "archive-cover") => void;
  onClosePanel: () => void;
  onUpload: (event: React.ChangeEvent<HTMLInputElement>) => Promise<void>;
  onSelectCover: (id: number | null) => void;
  onSelectArchiveCover: (id: number | null) => void;
...
export function CoverPanel({
  ...
  onUpload,
  onSelectCover,
  onSelectArchiveCover,
  onChangeSource,
}: CoverPanelProps) {
  const isCoverPanelOpen = activePanel?.kind === "cover";
  const isArchiveCoverPanelOpen = activePanel?.kind === "archive-cover";
  ...
  return (
    <EditorPanel title={<>🖼 Обложка</>}>
      <div className="grid gap-4">
        ...
        <MediaSummaryCard
          asset={assets.find((asset) => asset.id === form.archiveCover) ?? null}
          accept="image"
          emptyLabel="Обложка для архива"
          onOpen={() => onOpenPanel("archive-cover")}
          onClear={() => onSelectArchiveCover(null)}
          openLabel="Выбрать archive cover"
        />

        {isArchiveCoverPanelOpen ? (
          <div className="rounded border border-black/10 p-3 dark:border-white/10">
            <div className="mb-2 flex items-center justify-between">
              <h4 className="text-sm font-semibold">Выбор обложки для архива</h4>
              <button type="button" onClick={onClosePanel} className="text-xs text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200">Закрыть</button>
            </div>
            <MediaPicker
              assets={assets}
              accept="image"
              selectedId={form.archiveCover}
              onSelect={onSelectArchiveCover}
              onUpload={onUpload}
              uploadLabel="Загрузить новое изображение"
            />
          </div>
