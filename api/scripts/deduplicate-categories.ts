...
async function main() {
  ...
  const slugToCanonical = new Map<string, typeof allCategories[0]>();
  ...
  for (const dup of duplicates) {
    ...
    if (!DRY_RUN) {
      // Move content relations from duplicate to canonical
      await prisma.$executeRawUnsafe(
        `INSERT INTO "_CategoryToContentItem" ("A", "B")
         SELECT $1::uuid, "B"
      ...
      await prisma.$executeRawUnsafe(
        `DELETE FROM "_CategoryToContentItem" WHERE "A" = $1::uuid`,
        dup.id,
      );

      // Reparent children of duplicate to canonical
  ...
  // Replace duplicate slugs in JSON content fields
...
async function replaceSlugsInJsonFields(replacements: { from: string; to: string }[]) {
  ...
  for (const { table, column } of jsonColumns) {
    ...
    for (const record of records) {
      ...
      if (JSON.stringify(original) === JSON.stringify(replaced)) continue;
