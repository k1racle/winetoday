...
const idMaps = {
  ...
  author: new Map<number, string>(),
  category: new Map<number, string>(),
  tag: new Map<number, string>(),
  content: new Map<string, string>(), // `${type}:${oldId}`
};
...
async function resolveFileForComponent(relatedType: string, relatedId: number, field: string) {
  ...
  return res.rows.map((r) => idMaps.media.get(r.file_id)).filter(Boolean) as string[];
}
  ...
  for (const cmp of cmpRows.rows) {
    ...
    if (type === 'shared.seo') {
      const r = await strapi.query(
        `SELECT meta_title, meta_description, keywords, canonical_url, no_index, no_follow
         FROM components_shared_seos WHERE id = $1`,
        [cmpId],
      );
      const s = r.rows[0];
      if (s) {
    ...
    if (type === 'shared.source-link') {
      const r = await strapi.query(
        `SELECT name, url FROM components_shared_source_links WHERE id = $1`,
        [cmpId],
      );
      const s = r.rows[0];
    ...
    switch (type) {
      case 'blocks.html-editor': {
        const r = await strapi.query(
          `SELECT title, content FROM components_blocks_html_editors WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
      ...
      case 'blocks.rich-text': {
        const r = await strapi.query(
          `SELECT title, content FROM components_blocks_rich_texts WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
      ...
      case 'blocks.image-highlight': {
        const r = await strapi.query(
          `SELECT caption, credit FROM components_blocks_image_highlights WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
      ...
      case 'blocks.image-gallery': {
        const r = await strapi.query(
          `SELECT title, description FROM components_blocks_image_galleries WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
      ...
      case 'blocks.image-slider': {
        const r = await strapi.query(
          `SELECT title, description, photo_source FROM components_blocks_image_sliders WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
      ...
      case 'blocks.quote': {
        const r = await strapi.query(
          `SELECT text, author, role FROM components_blocks_quotes WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
        block.type = 'quote';
      ...
      case 'blocks.embed': {
        const r = await strapi.query(
          `SELECT title, html FROM components_blocks_embeds WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
        block.type = 'embed';
      ...
      case 'blocks.cta': {
        const r = await strapi.query(
          `SELECT eyebrow, title, description, theme FROM components_blocks_ctas WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
        block.type = 'cta';
      ...
      case 'blocks.hero': {
        const r = await strapi.query(
          `SELECT eyebrow, title, description, theme FROM components_blocks_heroes WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
        block.type = 'hero';
      ...
      case 'blocks.archive-feed': {
        const r = await strapi.query(
          `SELECT title, description, content_type, "limit" FROM components_blocks_archive_feeds WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
        ...
        const catRes = await strapi.query(
          `SELECT category_id FROM components_blocks_archive_feeds_categories_lnk WHERE archive_feed_id = $1`,
          [cmpId],
        );
      ...
      case 'blocks.link-grid': {
        const r = await strapi.query(
          `SELECT title, description FROM components_blocks_link_grids WHERE id = $1`,
          [cmpId],
        );
        const row = r.rows[0];
...
async function migrateContentItem(
  ...
  // Strapi v5 keeps multiple versions (draft/published) per document_id.
  // Keep one row per slug: prefer published, then most recently updated.
  ...
  for (const row of selected) {
    ...
    const authorRes = await strapi.query(
      `SELECT author_id FROM ${table}_author_lnk WHERE ${idCol} = $1`,
      [row.id],
    );
    ...
    const catRes = await strapi.query(
      `SELECT category_id FROM ${table}_categories_lnk WHERE ${idCol} = $1`,
      [row.id],
    );
    ...
    if (tagTableExists.rowCount && tagTableExists.rowCount > 0) {
      const tagRes = await strapi.query(
        `SELECT tag_id FROM ${table}_tags_lnk WHERE ${idCol} = $1`,
        [row.id],
      );
      tagIds = tagRes.rows
    ...
    // News-specific source fields -> sources JSONB
    if (row.source_name || row.source_url) {
      sources.push({ name: row.source_name, url: row.source_url });
    }

    // Video-specific fields -> first video-player block
    if (row.video_url) {
      blocks.unshift({
        id: randomUUID(),
        type: 'video-player',
...
async function migrateArticles() {
  await migrateContentItem(
    ContentType.article,
    'articles',
    'featured, pinned, homepage_lead, homepage_special_block, reading_time, material_label, editorial_status, submitted_at, reviewed_at, review_comment',
    (row) => ({
      status: row.editorial_status === 'published' ? ContentStatus.published : ContentStatus.draft,
...
async function migrateNews() {
  await migrateContentItem(
    ContentType.news,
    'news_items',
    'featured, pinned, homepage_lead, homepage_special_block, material_label, source_name, source_url',
    (row) => ({
      status: row.published_at ? ContentStatus.published : ContentStatus.draft,
      featured: row.featured,
      pinned: row.pinned,
      homepageLead: row.homepage_lead,
      homepageSpecialBlock: row.homepage_special_block,
...
async function migrateVideos() {
  await migrateContentItem(
    ContentType.video,
    'videos',
    'pinned, homepage_lead, homepage_special_block, material_label, video_url, duration',
    (row) => ({
      status: row.published_at ? ContentStatus.published : ContentStatus.draft,
      pinned: row.pinned,
      homepageLead: row.homepage_lead,
