...
async function buildAuthorIdMap() {
...
async function loadStrapiUsers() {
...
async function pickAuthorId(rows, authorIdMap) {
...
function resolveRole(rows, authorId) {
    const roleType = rows.find((r) => r.role_type)?.role_type;
    if (roleType === 'editor')
        return client_1.Role.editor;
    if (authorId)
        return client_1.Role.author;
...
async function migrateUsers() {
    ...
    for (const [, rows] of grouped) {
        ...
        if (authorId) {
            ...
            if (linked) {
                console.warn(`Author ${authorId} already linked to another user, skip link for ${base.email}`);
            }
        }
